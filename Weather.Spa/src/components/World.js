...
const RowTemperatureRenderer = ({ row, idx }) => {
    return (
            <div><span>{row.TemperatureDisplay} </span> <img src={row.Icon} width="40px" /></div>
    );
};
...
class World extends React.Component {
  ...
  componentWillMount()
  {
      this.props.actions.weatherActions.getWorldTemperatures();
  }

  render() {
      var columns = getColumns(this.props.actions);
      ...
      return (
         <TemperatureGrid rows={rows} columns={columns}  />
      )
    } 
}

export default connect(mapStateToProps, mapDispatchToPropsReposPage)(World);
